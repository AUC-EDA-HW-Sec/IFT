...
class IFT:
    def __init__(self, eblif_fileName):
        ...
        self.variable_names = self.eblif.variables + [name + "_t" for name in self.eblif.variables]
        self.vars = exprvars('v', len(self.variable_names))
        self.name_map = dict(zip(self.variable_names, self.vars))
...
if __name__ == "__main__":
    ift = IFT("simple_full_adder_2bit_v2.eblif")
    ift.run()
